...
The `ssh` command group allows you to manage SSH keys and configurations for your cloud machines. These commands help you set up and manage SSH access to your machines.
...
```bash
lazycloud ssh keys add <name> [options]
```
...
lazycloud ssh keys add my-key

# Add using specific key
lazycloud ssh keys add my-key --path ~/.ssh/custom_key.pub
```
...
```bash
lazycloud ssh keys rm <name>
```

#### Example

```bash
lazycloud ssh keys rm my-key
```
